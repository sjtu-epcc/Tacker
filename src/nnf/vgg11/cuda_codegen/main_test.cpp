...
int main(void){
    ...
    {
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_0_0, Parameter_0_0_host, sizeof(float) * 9633792, cudaMemcpyHostToDevice));
        kernel_entry(Parameter_0_0, &Result_99_0);
        CUDA_SAFE_CALL(cudaMemcpy(Result_99_0_host, Result_99_0,  sizeof(float) * 64064, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaDeviceSynchronize()); 
        printf("%s \n", "Result_99_0:");
        for (int i = 0; i < 10; ++i) printf("%e ", (float)Result_99_0_host[i]); 
        printf(" .. (size = 64064, ends with %e);\n", (float)Result_99_0_host[64063]);
    }
