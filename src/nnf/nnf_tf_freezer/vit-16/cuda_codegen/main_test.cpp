...
int main(int argc, char *argv[]){
    ...
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Parameter_207_0_host, sizeof(float)* 2408448));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Parameter_207_0, sizeof(float) * 2408448));

    //output arguments
    float* Result_1527_0_host, *Result_1527_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_1527_0_host, sizeof(float) * 2420736));

    // fill input values
    for (int i = 0; i < 2408448; ++i) Parameter_207_0_host[i] = 1.0f;


    //warm up for 5 iters:
    for(int i_=0; i_< 5; i_++)
    {
        CUDA_SAFE_CALL(cudaMemcpy(Parameter_207_0, Parameter_207_0_host, sizeof(float) * 2408448, cudaMemcpyHostToDevice));
        kernel_entry(Parameter_207_0, &Result_1527_0);
        CUDA_SAFE_CALL(cudaMemcpy(Result_1527_0_host, Result_1527_0,  sizeof(float) * 2420736, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        printf("%s \n", "Result_1527_0:");
        for (int i = 0; i < 10; ++i) printf("%e ", (float)Result_1527_0_host[i]); 
        printf(" .. (size = 2420736, ends with %e);\n", (float)Result_1527_0_host[2420735]);
    }
