...
from vgg_model import Vgg11Model, Vgg16Model, Vgg19Model
...
if args.model_name == 'resnet50':
    ...
    inputs = [graph_in]
    if args.const_folding:
        outputs = [tf.identity(tf.identity(logits, name="logi   ts"), name="logits_identity")]
    else:
        outputs = [tf.identity(logits, name="logits")]
...
#         outputs = [tf.identity(logits, name="logits")]
    ...
    batch_size = 8   # TODO 32 --> 100ms, 16 --> 87ms, 8 --> 85ms
    hidden_size = 256
    num_layer = 10
