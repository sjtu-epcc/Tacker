...
int main(void){
    ...
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Parameter_0_0_host, sizeof(float)* 7225344));
    CUDA_SAFE_CALL(cudaMalloc((void**)&Parameter_0_0, sizeof(float) * 7225344));

    //output arguments
    float* Result_505_0_host, *Result_505_0;
    CUDA_SAFE_CALL(cudaMallocHost((void**)&Result_505_0_host, sizeof(float) * 48048));

    // fill input values
    for (int i = 0; i < 7225344; ++i) Parameter_0_0_host[i] = 1.0f;

    CUDA_SAFE_CALL(cudaMemcpy(Parameter_0_0, Parameter_0_0_host, sizeof(float) * 7225344, cudaMemcpyHostToDevice));
    ...
    for(int i_=0; i_< 5; i_++)
    {
        ...
        CUDA_SAFE_CALL(cudaMemcpy(Result_505_0_host, Result_505_0,  sizeof(float) * 48048, cudaMemcpyDeviceToHost));
        CUDA_SAFE_CALL(cudaDeviceSynchronize()); 
        printf("%s \n", "Result_505_0:");
        for (int i = 0; i < 10; ++i) printf("%e ", (float)Result_505_0_host[i]); 
        printf(" .. (size = 48048, ends with %e);\n", (float)Result_505_0_host[48047]);
    }
