...
def extract_data(file_path):
    data = []
    pattern = r'--- (\d+) ---\s+load_ratio:\s+(\d+\.\d+)\s+mix_duration:\s+(\d+\.\d+)\s+.*sgemm_blk_num:\s+(\d+),\s+fft_blk_num:\s+(\d+)'
...
file_path = 'fft_sgemm_1_4.log'
data = extract_data(file_path)

# Replace 'output.xlsx' with your desired output Excel file name
output_file = 'fft_sgemm_load_ratio_data.xlsx'
